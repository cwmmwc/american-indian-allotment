...
|---|---:|---|---|
| `remarks_regex_v2` | 69,017 | Parse remarks for cross-references like `SEE SERIAL PATENT NR 75921-09 FOR FEE PATENT`, then validate the extracted accession against the patent catalog. v2 fixes a v1 regex bug that captured only the first accession in `NR X AND Y FOR FEE PATENT` patterns and silently dropped ~3,970 sibling refs. A 2026-05-26 re-run with a broadened scan filter (see bug history below) added 3,636 net new linkages including ~1,965 with un-mappable fee patents the original scan had silently skipped. | `parse_remarks_fee_refs.py` → `validate_remarks_extractions.py` |
...
| `vision_v5` | 197 | Vision-model read (Claude Sonnet 4.6, cross-validated with Qwen2.5-VL-72B) of the scanned trust-patent page surfaced a "Fee Patent Issued" stamp where remarks and parcel matching missed it. `fee_accession` is NULL on these rows — the v5 prompt confirms a conversion occurred but does not transcribe the handwritten fee-patent number. **Note:** ~112 of the 197 vision rows also have a `remarks_regex_v2` row for the same trust accession (independent dual-source confirmation); the remaining ~85 are visible only from the page scan. | `extract_annotations_v4.py` (Sonnet) + `extract_annotations_qwen_vision.py` (Qwen on HPC) → `extract_vision_v5_candidate_hidden.py` → `compare_sonnet_qwen_candidate_hidden.py` → `load_vision_v5_hidden_conversions.py` |

**Bug history — `remarks_regex_v2` scan filter, fixed 2026-05-26.** The original SQL filter used `all_patents.authority IN (trust-class list)` to select rows to scan. But the `all_patents` view aliases `document_class AS authority` for un-mappable patents (the UNION ALL branch where `blm_allotment_patents` has no match), so every un-mappable trust patent appeared with `authority='Serial Land Patent'` and failed the IN-list. ~9,400 trust patents were silently excluded from scanning, costing ~3,636 missed linkages corpus-wide. Surfaced by Bill Putawawa's trust `117036` (remarks: `SEE SERIAL PATENT NR 985005 FOR FEE PATENT`) being flagged as a "hidden" vision_v5 conversion when its fee pair was actually textually documented all along. Fix: scan also includes `objectid IS NULL AND NOT IN fee_patents`. See commit `7f9d4bc`.
